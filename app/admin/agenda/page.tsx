...
import { isOverdue, useAgendaData, usePersonalAgendaData, useAgendaStats, useCompanyData, useEmployeeData, useFilteredAgenda } from "./hooks";
...
export default function AgendaPage(){
 ...
 const {tasks:companyTasks,loading:companyLoading,loadError:companyLoadError,refresh:refreshCompany}=useAgendaData(selectedCompany?.id??"");
 const {tasks:personalTasks,loading:personalLoading,loadError:personalLoadError,refresh:refreshPersonal}=usePersonalAgendaData();
 ...
 const scopedTasks=scope==="MINE"?personalTasks:companyTasks;
 const loading=scope==="MINE"?personalLoading:companyLoading;
 const loadError=scope==="MINE"?personalLoadError:companyLoadError;
 async function refresh(){await Promise.all([refreshCompany(),refreshPersonal()])}
 ...
 async function handleCreate(e:FormEvent){e.preventDefault();if(!selectedCompany)return setError("Firma seçilmelidir.");if(!selectedCompany.localId||selectedCompany.localId<=0)return setError("Seçilen firmanın mobil firma ID bilgisi eksik.");if(!title.trim())return setError("Görev başlığı zorunludur.");try{setSaving(true);setError("");await createAgenda({firm_id:selectedCompany.localId,web_firm_id:selectedCompany.id,title:title.trim(),note:note.trim()||null,type,priority:Number(priority),due_at:dueAt?new Date(dueAt).toISOString():null,end_at:endAt?new Date(endAt).toISOString():null,location:location.trim()||null,meeting_link:meetingLink.trim()||null,assigned_to:scope==="MINE"?null:selectedEmployee?.full_name||null,assigned_employee_remote_id:scope==="MINE"?null:selectedEmployee?.id||null,assigned_employee_local_id:scope==="MINE"?null:selectedEmployee?.local_employee_id??null,is_all_day:isAllDay,personal:scope==="MINE"});clear();setShowCreate(false);setMessage("Ajanda kaydı oluşturuldu.");await refresh()}catch(v){setError(v instanceof Error?v.message:"Kayıt oluşturulamadı.")}finally{setSaving(false)}}
 async function toggle(t:AgendaTask){if(t.source_readonly||t.source!=="WEB")return;const firmId=t.web_firm_id||selectedCompany?.id;if(!firmId)return;try{await patchAgenda(t.id,firmId,{status:t.status===1?0:1,progress:t.status===1?0:100});setMessage(t.status===1?"Görev tekrar açıldı.":"Görev tamamlandı.");await refresh()}catch(v){setError(v instanceof Error?v.message:"Görev güncellenemedi.")}}
 async function del(t:AgendaTask){const firmId=t.web_firm_id||selectedCompany?.id;if(!firmId||!confirm(`“${t.title}” kaydı silinsin mi?`))return;try{await removeAgenda(t.id,firmId);setMessage("Ajanda kaydı silindi.");await refresh()}catch(v){setError(v instanceof Error?v.message:"Kayıt silinemedi.")}}
 ...
 return <main style={{...page,padding:mobile?"14px 12px 44px":"28px 30px 60px"}}><div style={{maxWidth:1540,margin:"0 auto",width:"100%",minWidth:0}}>
  ...
  <section style={{...toolbar,display:"grid",gridTemplateColumns:mobile?"1fr":"minmax(280px,360px) auto auto 1fr",alignItems:"end",gap:mobile?10:16}}>
   ...
   <div style={{...segmented,width:mobile?"100%":"auto"}}><button style={{...seg(scope==="COMPANY"),flex:1}} onClick={()=>setScope("COMPANY")}>Firma Ajandası</button><button style={{...seg(scope==="MINE"),flex:1}} onClick={()=>setScope("MINE")}>Benim Ajandam</button></div>
   ...
   <div style={{marginLeft:mobile?0:"auto",fontSize:12,color:muted,overflowWrap:"anywhere"}}>{scope==="MINE"?`Kişisel Ajanda • ${companies.length} aktif yetkili firma`:`Firma kilitli görünüm • ${selectedCompany?.name||"Firma seçiliyor"}`}</div>
  </section>
