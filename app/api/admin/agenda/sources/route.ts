 ...
 try{
  ...
  if(canOperational){
   ...
   if(rule.minutes>0){
    ...
    if(missingCount>0){records.push(base(firmId,"TRAINING_LEGAL_GAP","legal-gap",{title:`Yasal Eğitim Açığı • ${missingCount} çalışan`,note:`${rule.label} sınıfı için kişi başı ${rule.minutes/60} saat yasal eğitim gerekliliği. Toplam ${Math.ceil(missingMinutes/60)} saat eğitim açığı bulunuyor.`,type:"TRAINING",category:"TRAINING_LEGAL_GAP",due_at:new Date().toISOString(),priority:2,module_ref:"TRAINING_LEGAL_GAP",module_remote_id:"legal-gap",source_url:"/admin/training"}))}
    if(expiringCount>0){records.push(base(firmId,"TRAINING_RENEWAL","legal-renewal",{title:`Eğitim Geçerliliği Yaklaşıyor • ${expiringCount} çalışan`,note:`Önümüzdeki 45 gün içinde ${expiringCount} çalışanın geçerli eğitim süresi yasal eşiğin altına düşecek.`,type:"TRAINING",category:"TRAINING_RENEWAL",due_at:(earliestExpiry||new Date(horizon)).toISOString(),priority:1,module_ref:"TRAINING_RENEWAL",module_remote_id:"legal-renewal",source_url:"/admin/training"}))}
   }
  }
