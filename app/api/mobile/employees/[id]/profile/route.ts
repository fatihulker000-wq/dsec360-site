...
async function getProfileAccessContext(request: Request):
  Promise<ProfileAccessContext> {
  const apiKey = String(request.headers.get("x-api-key") || "").trim();
  const url = new URL(request.url);
  const companyId = String(url.searchParams.get("firmId") || "").trim();

  const allowed = apiKey === MOBILE_API_KEY && Boolean(companyId);
  ...
  return {
    ...
    companyId,
    companyScoped: true,
    // Mobil uygulamada sağlık içeriği varsayılan olarak metadata-only tutulur.
    canViewSensitiveHealth: false,
  };
}
...
export async function GET(
  ...
  try {
    ...
    const access =
      await getProfileAccessContext(request);
