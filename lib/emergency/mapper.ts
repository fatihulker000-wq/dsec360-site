...
export function mapPlan(
  ...
  return {
    ...
    planDateMillis: toMillis(
      row.plan_date_millis,
      Date.now()
    )!,

    validUntilMillis: toMillis(
      row.valid_until_millis
    ),

    revisionDateMillis: toMillis(
      row.revision_date_millis
    ),
    ...
    emergencyExitRoutePhotoUri:
      row.emergency_exit_route_photo_uri
        ? String(
            row.emergency_exit_route_photo_uri
          )
        : null,
    ...
    accidentScenario: String(
      row.accident_scenario || ""
    ),

    evacuationScenario: String(
      row.evacuation_scenario || ""
    ),

    createdAtMillis: toMillis(
      row.created_at,
      Date.now()
    )!,

    updatedAtMillis: toMillis(
      row.updated_at,
      Date.now()
    )!,
  };
...
export function mapDrill(
  ...
  return {
    ...
    drillDateMillis: toMillis(
      row.drill_date_millis,
      Date.now()
    )!,

    nextDrillDueMillis: toMillis(
      row.next_drill_due_millis
    ),
...
export function planToRow(
  ...
  return {
    ...
    plan_date_millis: Number(
      record.planDateMillis ||
        Date.now()
    ),
    ...
    revision_no: String(
      record.revisionNo || "R0"
    ),

    assembly_area: String(
      record.assemblyArea || ""
    ),

    emergency_coordinator: String(
      record.emergencyCoordinator || ""
    ),
    ...
    assembly_area_photo_uri:
      record.assemblyAreaPhotoUri
        ? String(
            record.assemblyAreaPhotoUri
          )
        : null,
    ...
    emergency_exit_route_photo_uri:
      record.emergencyExitRoutePhotoUri
        ? String(
            record.emergencyExitRoutePhotoUri
          )
        : null,

    fire_equipment_photo_uri:
      record.fireEquipmentPhotoUri
        ? String(
            record.fireEquipmentPhotoUri
          )
        : null,

    emergency_board_photo_uri:
      record.emergencyBoardPhotoUri
        ? String(
            record.emergencyBoardPhotoUri
          )
        : null,
    ...
    earthquake_scenario: String(
      record.earthquakeScenario || ""
    ),

    flood_scenario: String(
      record.floodScenario || ""
    ),

    accident_scenario: String(
      record.accidentScenario || ""
    ),

    evacuation_scenario: String(
      record.evacuationScenario || ""
    ),
...
export function drillToRow(
  ...
  return {
    ...
    drill_date_millis: Number(
      record.drillDateMillis ||
        Date.now()
    ),

    next_drill_due_millis:
      record.nextDrillDueMillis ===
        null ||
      record.nextDrillDueMillis ===
        undefined
        ? null
