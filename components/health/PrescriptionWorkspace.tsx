...
import MedulaPrescriptionActions from "./prescription/MedulaPrescriptionActions";
...
export default function PrescriptionWorkspace({employee}:Props){
 ...
 function selectDrug(i:number,d:any){
  upd(i,"medicineName",d.drug_name||d.name||"");
  upd(i,"activeIngredient",d.active_ingredient||"");
  upd(i,"dosage",d.strength||"");
  upd(i,"usageType",d.dosage_form||"");
  if(d.barcode||d.barkod)upd(i,"barcode",String(d.barcode||d.barkod));
  setDrug(p=>({...p,[i]:[]}))
 }
 ...
 return <div style={{display:"grid",gap:14}}>
   ...
   <section style={card}>
    ...
    {!diagnosisManual ? <div style={{display:"grid",gap:10,marginTop:10}}>
      ...
      <Field label="ICD-10 kodu veya hastalık adıyla ara">
        ...
        {icd.length>0&&<div style={drop}>
          {icd.slice(0,20).map((x:any)=><button
            ...
            style={option}
            onClick={()=>{
              setDiagnosisCode(String(x.code||""));
              setDiagnosisName(String(x.name||""));
              setDiagnosisQuery(`${x.code||""}${x.code&&x.name?" — ":""}${x.name||""}`);
              setIcd([]);
            }}
          ><b>{x.code||"-"}</b> — {x.name||"-"}</button>)}
        </div>}
      </Field>
      ...
      <Field label="ICD-10 (opsiyonel)">
        <input value={diagnosisCode} onChange={e=>setDiagnosisCode(e.target.value.toUpperCase())} placeholder="Örn: K29.7" style={input}/>
      </Field>
      <Field label="Tanı / Hastalık Adı">
        <input value={diagnosisName} onChange={e=>{setDiagnosisName(e.target.value);setDiagnosisQuery(e.target.value)}} placeholder="Manuel tanı / hastalık adı" style={input}/>
      </Field>
    </div>}
   </section>
   ...
   <section style={card}>
    <div style={head}>
      <div>
        <h3 style={h3}>İlaçlar / MEDULA Kullanım Bilgileri</h3>
        <div style={muted}>Sistem ilaç havuzundan seçebilir veya bulunmayan ilacı manuel yazabilirsin.</div>
      </div>
      <button style={btn} onClick={()=>setItems(p=>[...p,{...blank}])}>+ İlaç</button>
    </div>
    <div style={{display:"grid",gap:10}}>
      {items.map((x,i)=><div key={i} style={medicine}>
       ...
       <div style={grid}>
        <Field label="İlaç adı / ilaç havuzu">
          <input
            value={x.medicineName}
            onChange={e=>searchDrug(i,e.target.value)}
            placeholder="İlaç adı, barkod veya etkin madde ara..."
            style={input}
          />
          {drug[i]?.length>0&&<div style={drop}>
            {drug[i].slice(0,15).map((d:any)=><button
              type="button"
              key={d.id||d.barcode||d.barkod||d.drug_name||d.name}
              style={option}
              onClick={()=>selectDrug(i,d)}
            >
              <b>{d.drug_name||d.name||"-"}</b>
              {d.active_ingredient?` — ${d.active_ingredient}`:""}
              {(d.barcode||d.barkod)?` • ${d.barcode||d.barkod}`:""}
            </button>)}
          </div>}
          {x.medicineName.trim().length>=2&&(!drug[i]||drug[i].length===0)&&
            <div style={manualHint}>Havuzda yoksa yazdığın ilaç adı <b>manuel</b> olarak kaydedilir.</div>}
        </Field>
   ...
   {id&&<MedulaPrescriptionActions prescriptionId={id} ePrescriptionNo={eNo} patientTc={patientTc} medulaStatus={medulaStatus} onChanged={({status,message}:{status?:string;message?:string})=>{if(status)setMedulaStatus(status);if(message)setMedulaResponse(message);setRefresh(x=>x+1)}}/>}
